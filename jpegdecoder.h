...
//------------------------------------------------------------------------------
// idct.cpp
void idct(const jpgd_block_t *data, uint8 *Pdst_ptr, int block_max_zag);
void idct_4x4(const jpgd_block_t* data, uint8* Pdst_ptr);
//------------------------------------------------------------------------------
// fidctfst.cpp
...
//------------------
  ...
  uint8 m_huff_ac[JPGD_MAXHUFFTABLES];
  uint8* m_huff_num[JPGD_MAXHUFFTABLES];  /* pointer to number of Huffman codes per bit size */
  uint8* m_huff_val[JPGD_MAXHUFFTABLES];  /* pointer to Huffman codes per bit size */
  ...
  uint8 m_padd_1[128];
  uint8 m_in_buf[JPGD_INBUFSIZE + 128];
  uint8 m_padd_2[128];
  ...
  uint  m_last_dc_val[JPGD_MAXCOMPONENTS];
