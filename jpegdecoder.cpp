...
void jpeg_decoder::read_dht_marker(void)
{
  ...
  while (left)
  {
    ...
    for (i = 1; i <= 16; i++)
    {
      m_huff_num[i] = static_cast<uint8>(get_bits_1(8));
      count += m_huff_num[i];
    }
    ...
    for (i = 0; i < count; i++)
      m_huff_val[i] = static_cast<uint8>(get_bits_1(8));
    ...
    if (!this->m_huff_num[index])
      this->m_huff_num[index] = (uint8 *)alloc(17);

    if (!this->m_huff_val[index])
      this->m_huff_val[index] = (uint8 *)alloc(256);
...
void jpeg_decoder::read_sof_marker(void)
{
  ...
  if (left != (uint)(m_comps_in_frame * 3 + 8))
  ...
  Phuff_tables_t hs)
{
  ...
  while (p < lastp)
  {
    ...
    hs->code_size[i] = static_cast<uint8>(code_size);
...
void jpeg_decoder::init_frame(void)
{
  ...
  m_pScan_line_0         = (uint8 *)alloc(m_dest_bytes_per_scan_line + 8);
  memset(m_pScan_line_0, 0, m_dest_bytes_per_scan_line);

  m_pScan_line_1         = (uint8 *)alloc(m_dest_bytes_per_scan_line + 8);
  ...
  q = (uint8 *)alloc(m_max_blocks_per_mcu * 64 * sizeof(jpgd_block_t) + 8);
  ...
  if (m_freq_domain_chroma_upsample)
    m_pSample_buf = (uint8 *)(((uint)alloc(m_expanded_blocks_per_row * 64 + 8) + 7) & ~7);
  else
    m_pSample_buf = (uint8 *)(((uint)alloc(m_max_blocks_per_row * 64 + 8) + 7) & ~7);
  ...
  int block_len_x, int block_len_y)
{
  ...
  cb->Pdata = (uint8 *)alloc(cb->block_size * block_num_x * block_num_y);
